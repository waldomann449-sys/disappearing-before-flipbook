...
const pageFlip = new St.PageFlip(bookElement, {
  ...
  startZIndex:10,autoSize:true,maxShadowOpacity:.4,showCover:true,
...
function updateControls(){
  ...
  pageStatus.textContent=currentPage===0?`封面 · ${num} / ${count}`:currentPage>=last?`封底 · ${num} / ${count}`:portrait?`${num} / ${count}`:`${num}—${String(Math.min(currentPage+2,pages.length)).padStart(2,'0')} / ${count}`;
...
function turn(direction){
  if(isTurning||(direction<0&&currentPage===0)||(direction>0&&currentPage===pages.length-1))return;
...
window.addEventListener('keydown',event=>{
  if(event.altKey||event.ctrlKey||event.metaKey||isTurning||event.target.isContentEditable||/INPUT|SELECT|TEXTAREA/.test(event.target.tagName))return;
...
function showMessage(text){readerMessage.textContent=text;readerMessage.hidden=false;}
...
for(const img of bookElement.querySelectorAll('img')){
  img.addEventListener('error',()=>showMessage('有一页暂时未能载入，请刷新重试，或下载 PDF 阅读。'));
}
